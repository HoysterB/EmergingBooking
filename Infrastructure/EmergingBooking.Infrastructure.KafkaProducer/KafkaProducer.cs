...
internal class KafkaProducer<TKeyType, TEntity> : IDisposable
{
    ...
    {
        _kafkaRetryPolicy =
            Policy.HandleResult<DeliveryResult<TKeyType, TEntity>>(e => e == null)
    ...
    {
        using var producer = new ProducerBuilder<TKeyType, TEntity>(_producerConfig).SetKeySerializer(new JsonSerializerUTF8<TKeyType>()).SetValueSerializer(new JsonSerializerUTF8<TEntity>()).Build();
