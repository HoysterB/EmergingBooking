    ...
    where TEventType : class
{
    ...
    private readonly JsonCreationConverter<TEventType> _jsonCreationConverter;

    public KafkaConsumer(string group, string server, string topicName, JsonCreationConverter<TEventType> jsonCreationConverter)
    {
        _kafkaRetryPolicy = Policy.HandleResult<ConsumeResult<TKeyType, TEventType>>(r => r == null)
    ...
    public async Task ConsumeAsync(CancellationToken cancellationToken)
    {
        using (var consumer = new ConsumerBuilder<TKeyType, TEventType>(_consumerConfig)
                   ...
                   .SetValueDeserializer(new JsonDeserializerValueUTF8<TEventType>(_jsonCreationConverter))
                   .Build())
        {
